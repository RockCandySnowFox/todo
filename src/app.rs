...
impl MyApp {
    pub fn new(cc:&eframe::CreationContext) -> Self {
        ...
        to_do::setup_custom_fonts(&cc.egui_ctx);
        data.del = -1;
        data
    }
    ...
    fn show_active(&mut self, ui: &mut eframe::egui::Ui, active: bool) {
        let datae = self.data.iter().filter(|u| u.active == active).collect::<Vec<_>>();
    ...
    fn dataupdate(&mut self) {
        let datae = self.data.iter().filter(|u| u.active == false).collect::<Vec<_>>();
        self.task = datae.len();
        if self.del > -1 {
...
impl eframe::App for MyApp {
    fn update(&mut self, ctx: &eframe::egui::Context, _frame: &mut eframe::Frame) {
        eframe::egui::CentralPanel::default().show(ctx, |ui|{
            ui.heading("Todo");
            let p = ui.add(eframe::egui::TextEdit::singleline(&mut self.addtodo).hint_text("Write something here"));
